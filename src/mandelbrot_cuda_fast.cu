...
__device__ float mandelbrot_smooth_color_fast(float x, float y, int max_iter, float escape_radius_squared) {
    float zr = x, zi = y;
    float zr2 = zr * zr, zi2 = zi * zi;
    int iter = 0;
...
__global__ void mandelbrot_kernel_fast(float x_min, float y_min, float x_max, float y_max, int width, int height, int max_iter, float escape_radius_squared, int* results) {
    ...
    if (idx < width && idy < height) {
        ...
        results[idy * width + idx] = mandelbrot_escape_time_fast(real, imag, max_iter, escape_radius_squared);
    }
}

__global__ void mandelbrot_kernel_smooth_fast(float x_min, float y_min, float x_max, float y_max, int width, int height, int max_iter, float escape_radius_squared, float* results) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int idy = blockIdx.y * blockDim.y + threadIdx.y;
    if (idx < width && idy < height) {
        float pixel_size_x = (x_max - x_min) / width;
        ...
        results[idy * width + idx] = mandelbrot_smooth_color_fast(real, imag, max_iter, escape_radius_squared);
    }
}
...
extern "C" void compute_grid_cuda_smooth_fast(float x_min, float y_min, float x_max, float y_max, int width, int height, int max_iter, float escape_radius_squared, float* results) {
    ...
    mandelbrot_kernel_smooth_fast<<<num_blocks, threads_per_block>>>(x_min, y_min, x_max, y_max, width, height, max_iter, escape_radius_squared, d_results);
